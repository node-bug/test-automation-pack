...
function testRail() {
  ...
  that.getProjectByName = async (projectName) => (await my.connection
    .getProjects()).body.filter((project) => project.name === projectName)[0];

  // Test Suites and Cases
  // Takes in a project id and a name of the suite.
  // Returns the first instance of the suite.
  that.getSuiteByName = async (projectId, suiteName) => (await my.connection
    .getSuites(projectId)).body.filter((suite) => suite.name === suiteName)[0];

  // Takes in a project id, suite id, and a name of the section.
  // Returns the first instance of the section.
  that.getSectionByName = async (projectId, suiteId, sectionName) => (await my.connection
    .getSections(projectId, { suite_id: suiteId })).body
  ...
  that.getCaseByName = async (projectId, suiteId, sectionId, caseName) => (await my.connection
    .getCases(projectId, { suite_id: suiteId, section_id: sectionId })).body
    .filter((myCase) => myCase.title === caseName)[0];
  ...
  that.getTestRunByName = async (projectId, runName) => (await my.connection
    .getRuns(projectId)).body.filter((run) => run.name === runName)[0];

  // Takes in a plan id and the name of the case.  Returns the first instance of the test run.
  that.getTestByName = async (runId, caseName) => (await my.connection
    .getTests(runId, {})).body.filter((test) => test.title === caseName)[0];

  that.addTestRun = async (projectId, suiteId, runName) => {
    const run = await that.getTestRunByName(projectId, runName);
    if (run === undefined) {
      await my.connection
        .addRun(projectId, { suite_id: suiteId, name: runName, include_all: true });
    }
  };
